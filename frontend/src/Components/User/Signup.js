...
const Signup = () => {
  ...
  const [ user, setUser ] = useState({ name:"",email:"",password:"",passwordConfirm:"",phoneNumber:"" });
  const { password, passwordConfirm } = user;
  const onChange = (e) => { setUser({...user, [e.target.name]: e.target.value}) };
  const submitHandler = async(e) => {
    e.preventDefault();
    if(password!== passwordConfirm){
      toast.error("Both Passowrd do not match! Check & Try Again..");
      return;
    }
          ...
          {["name", "email", "password", "passwordConfirm", "phoneNumber"].map(
            (field)=>(
              <div className='form-group' key={field}>
                ...
                <input type={field.includes("password")? "password" : "text"} id={`${field}_field`}
                  className='form-control' name={field} value={user[field]} onChange={onChange} />
              </div>
            )
          )}
